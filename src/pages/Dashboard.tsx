    ...
    <AppLayout pageId="dashboard">
      ...
      <div className="mb-8 grid grid-cols-2 gap-4">
        <Link to="/new" className="block">
          <button className="w-full h-28 rounded-2xl bg-primary text-primary-foreground flex items-center justify-center gap-3 text-xl font-extrabold btn-glow transition-all duration-200 active:scale-[0.97] hover:brightness-110">
            <PlusCircle className="h-7 w-7" />
            Nova Entrada
          </button>
        </Link>
        <Link to="/exit" className="block">
          <button className="w-full h-28 rounded-2xl bg-success text-success-foreground flex items-center justify-center gap-3 text-xl font-extrabold btn-glow-success transition-all duration-200 active:scale-[0.97] hover:brightness-110">
      ...
      <div className="mb-8">
        <h2 className="text-base font-bold text-foreground mb-3 tracking-tight">
        ...
        {activeLoading ? (
          ...
          <div className="apple-card p-8 text-center">
            <p className="text-sm font-medium text-muted-foreground">Nenhum visitante ativo no momento.</p>
          </div>
        ) : (
          <div className="grid gap-3 sm:grid-cols-2">
            {activeVisitors.map((log) => {
              ...
              return (
                <div key={log.id} className="apple-card p-4 flex items-center gap-4 transition-all hover:shadow-lg animate-fade-in">
                  <button onClick={() => openSheet(log)} className="shrink-0">
                    {resident?.photo_url ? (
                      <img src={resident.photo_url} alt={log.driver_name} className="h-14 w-14 rounded-2xl object-cover border-2 border-primary/20 shadow-sm" />
                    ) : (
                  ...
                  <button onClick={() => openSheet(log)} className="flex-1 min-w-0 text-left">
                    <p className="text-3xl font-extrabold text-foreground leading-none tracking-tight">{log.destination}</p>
                    <p className="text-sm font-bold text-foreground/70 mt-1 uppercase truncate">{log.driver_name}</p>
                    {log.plate && (
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-xs font-medium text-muted-foreground">{formatTime(log.entry_time)}</p>
                  ...
                  <div className="shrink-0 flex flex-col gap-2">
                    <button
                      ...
                      className="h-11 w-11 rounded-xl bg-success/10 flex items-center justify-center hover:bg-success/20 transition-colors"
                      title="Dar saída"
                    >
                    ...
                    <button
                      ...
                      className="h-11 w-11 rounded-xl bg-primary/8 flex items-center justify-center hover:bg-primary/15 transition-colors"
                      title="Editar"
                    >
      ...
      <div>
        ...
        {!todayLogs?.length ? (
          <div className="apple-card p-8 text-center">
            <p className="text-sm font-medium text-muted-foreground">Nenhum registro hoje.</p>
          </div>
        ) : (
          <div className="space-y-1.5">
            {todayLogs.map((log) => (
              <div
                key={log.id}
                className="w-full rounded-xl bg-card border border-border/60 px-4 py-2.5 transition-all hover:shadow-md"
              ...
              >
                ...
                <div className="hidden sm:flex items-center gap-3">
                  <button onClick={() => openSheet(log)} className="flex-1 min-w-0 flex items-center gap-3 text-left">
                    ...
                    <span className="text-lg font-extrabold text-foreground w-12 shrink-0">{log.destination}</span>
                    <span className="text-sm font-semibold text-foreground/70 uppercase truncate flex-1">{log.driver_name}</span>
                  ...
                  <div className="shrink-0 flex items-center gap-1.5">
                    {!log.exit_time && (
                      <button onClick={() => setExitTarget(log)} className="h-8 w-8 rounded-lg bg-success/10 flex items-center justify-center hover:bg-success/20 transition-colors" title="Dar saída">
                        <LogOutIcon className="h-4 w-4 text-success" />
                      </button>
                    )}
                    <button onClick={() => openSheet(log)} className="h-8 w-8 rounded-lg bg-primary/8 flex items-center justify-center hover:bg-primary/15 transition-colors" title="Editar">
                ...
                <button onClick={() => openSheet(log)} className="sm:hidden w-full text-left">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-foreground">{formatTime(log.entry_time)}</span>
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {!log.exit_time && (
                        <button onClick={(e) => { e.stopPropagation(); setExitTarget(log); }} className="h-7 w-7 rounded-lg bg-success/10 flex items-center justify-center" title="Dar saída">
